...
- Adaptability: ライト/ダークの両テーマを `html[data-theme]` で切り替え、ヘッダーのアイコンボタンから選択できるようにする。
...
- Color
  ...
  - ライトテーマは Apple サイトのような白〜薄グレー面を基準に、`#1d1d1f` 系の本文色と `#0071e3` 系アクセントへトークンを上書きする。
- Type
...
- Theme Toggle: Header icon button. Default is light mode. Selected theme is stored in `localStorage`.
- Home Hero: Business Growth title + latest event panel. タイトルは最大88pxに抑える。
